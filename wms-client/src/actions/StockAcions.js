...
export const getOutStockToStoreOrder = (outStockOrderNo) => async (
  dispatch
) => {
  const res = await API.get(
    `/api/stock/queryOrder/outStockToStore/${outStockOrderNo}`
  );
...
export const stockIdentifiersAreShiped = (productNo, shipRequests) => async (
  dispatch
) => {
  try {
...
export const stockIdentifierIsNotShiped = (stockIdentifierId) => async (
  dispatch
) => {
  try {
...
export const stockIdentifierWaitToShrinkIsTrue = (stockIdentifierId) => async (
  dispatch
) => {
  try {
...
export const stockIdentifierWaitToShrinkIsFalse = (stockIdentifierId) => async (
  dispatch
) => {
  try {
...
export const batchCreateStockInfoesForShrink = (shrinkStockRequest) => async (
  dispatch
) => {
  try {
...
export const getInStockRollbackList = (orderType, orderNo) => async (
  dispatch
) => {
  try {
    const res = await API.get(
...
export const inStockRollback = (idList, orderType, orderNo) => async (
  dispatch
) => {
  try {
...
export const shrinkRollback = (stockIdentifierId, productNo) => async (
  dispatch
) => {
  try {
...
export const deleteOutStockRequest = (outStockRequestId) => async (
  dispatch
) => {
  try {
    const res = await API.patch(
...
export const getWaitHandleListWithInterval = (startDate, endDate) => async (
  dispatch
) => {
  const res = await API.get(
...
export const updateOutStockRequests = (outStockUpdateRequest) => async (
  dispatch
) => {
  try {
